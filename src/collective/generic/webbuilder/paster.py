        ...
        for template in templates:
            ...
            eps = [a
                   for a in pkg_resources.iter_entry_points(
                       'paste.paster_create_template', template.name)]
            if eps:
                ep = eps[0].load()
                ...
                for var in ep.vars:
                    ...
                    for group_data in groups_data:
                        if group_data['name'] != 'default' and not skip:
                            ...
                            if (
                                not found
                                and not var in template_data['added_options']
                            ):
                                not_explicit_options[var.name] = (var,
                                                                  'default',
                                                                  None,
                                                                  None)
                                option = None
                                ...
                                if option.default:
                                    ...
                                    for b in ['true', 'y', 'on']:
                                        if option.default.startswith(
                                            ...
                                            type = 'boolean'
                                if alias:
                                    template_data['aliases'].append((
                                        var, alias, var.default))
                                option.type = type
                                if (
                                    (not name in p.added_options)
                                    and (not name
                                         in template_data['added_options'])
                                ) or alias:
                                    if (
                                        not alias
                                        or (
                                            alias
                                            and (not alias in p.added_options)
                                        )
                                    ):
                                        group_data['options'].append(
                                            (var, option.type, alias, option))
                                        template_data['added_options'].append(
                                            name)
                                        # add the option globally only
                                         #and only if the option is not aliased
                                        if not alias:
                                            p.added_options.append(name)
                                        if alias:
                                            p.added_options.append(alias)
                                        # first option added which is not
                                        # hidden
                        ...
                        for name in (
                            p.added_options + template_data['added_options']
                        ):
                            if name in not_explicit_options:
                                del not_explicit_options[name]
                    # after all groups had been parsed, just mark the default
                    # options as added
